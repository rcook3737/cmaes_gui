...
function [xmin,stats_struct]=CMAES_norm_v3(filename,sigma,generations,N)   % (mu/mu_w, lambda)-CMA-ES
% --------------------  Initialization --------------------------------
load(filename)
trial = '1';
...
if counteval == 0
    if (N==4)
        paramNames = ["Ext_FFFB";"Ext_Gain";"Flx_FFFB";"Flx_Gain"];
        paramMeans = [         0;       0.5;         0;       0.5];
        paramMins  = [         0;         0;         0;         0];
        paramMaxes = [         1;         1;         1;         1];
    elseif (N==2)
        paramNames = ["Stiffness";"Ref_Angle"];
        paramMeans = [       1.2;         .4];
        paramMins  = [       0.5;          .05];
        paramMaxes = [        2;         .85];
    end
    ...
    stats_struct = struct('gen_idx',[],'x',zeros(lambda,N),'metrates',zeros(lambda,1),...
...
if N == 2
    ...
    subplot(N,1,1)
    scatter(0,paramMeans(1),40,colors(1,:))
    hold on
    ylabel('Stiffness')
%     ylim(variable1_range_norm)

    subplot(N,1,2)
    scatter(0,paramMeans(2),40,colors(1,:))
    hold on
...
while counteval <= stopeval-1
    ...
    if N == 2
        sprintf('Mean: %.3f Stiffness %.3f Ref. Ang\n',paramMean_norm.*paramRange_size)
    end
...
%         x_log(:,counteval) = x(:,k);
    ...
    [metrates] = metabolic_rate_estimation_NMM_v8(x,gen_num,paramNames,fileInfo,'1_25mps',N);
...
%     end
    ...
    stats_struct(gen_num).gen_idx = gen_num;
    stats_struct(gen_num).x_std = sigma;
    stats_struct(gen_num).min_metrate = sorted_metrates(1);
    stats_struct(gen_num).x_mean = paramMean_norm.*paramRange_size;
    stats_struct(gen_num).xmean_norm = paramMean_norm;
    stats_struct(gen_num).sigma = sigma;
    stats_struct(gen_num).x_std = std(paramRange_size.*x+paramRange(:,1),0,2);
    stats_struct(gen_num).x = paramRange_size.*x+paramRange(:,1);
    stats_struct(gen_num).metrates = metrates;
    stats_struct(gen_num).pc = pc;
    stats_struct(gen_num).ps = ps;
    % Candidate Plotting
    if N == 2
        figure(CMA_params)
        subplot(N,1,1)
        scatter(repmat(gen_num,1,lambda),x(1,:),40,colors(gen_num,:))
        scatter(gen_num,stats_struct(gen_num).x_mean(1),50,'k')        
        ylim(paramRange(1,:))
        subplot(N,1,2)
        scatter(repmat(gen_num,1,lambda),x(2,:),40,colors(gen_num,:))
        scatter(gen_num,stats_struct(gen_num).x_mean(2),50,'k')    
        ylim(paramRange(2,:))
...
%%%%%%% Evolution paths need to update each variable correctly
    ...
    stats_struct(gen_num).New_x_mean = paramMean_norm.*paramRange_size;
    stats_struct(gen_num).New_x_mean_norm = paramMean_norm;
    stats_struct(gen_num).New_sigma = sigma;
    stats_struct(gen_num).New_pc = pc;
    stats_struct(gen_num).New_ps = ps;
