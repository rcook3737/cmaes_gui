...
function [estimated_dot_E] = metabolic_rate_estimation_NMM_v8(x,gen_num,variables,fileInfo,speed,N)
%     x = 1;
...
for c = 1:lambda % testing for each candidate set
        ...
        % Display testing parameters for metronome implementation
        if N == 4
...
%     save(sprintf('metabolic_rate_est_NMM_4D_Habit_Emily_11222019_gen1_1_%s_Opt',speed),'t','vco2_rate','vo2_rate','x_star','mean_squared_error','y_bar','y_meas')
    ...
    save(sprintf('metabolic_rate_est_%s_%s_%s_gen%d_%d_speed125.mat',fileInfo.controller,...
        fileInfo.subjectID,fileInfo.date,gen_num,c),'t','vco2_rate','vo2_rate','x_star','mean_squared_error','y_bar','y_meas')
end
